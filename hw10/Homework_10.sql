...
-- This returs yes/no columns -- create view last_name_count_greatertwo as select last_name, count(*) > 2 FROM actor group by last_name;
...
select s.staff_id, s.first_name, s.last_name, p.staff_id, p.amount, p.payment_date sum(p.amount) as 'Total Amount Rung Up' 
from staff s  
join payment p 
...
select f.title, count(fa.actor_id) as 'Numner of Actors' 
from film f 
...
SELECT f.title, f.film_id, count(rental_id) as 'Most Frequent Rented Movies' FROM film f 
...
SELECT st.store_id, sum(p.amount) as 'Revenue' FROM payment p  
JOIN staff s
